            ...
            with open("%s.out.csv" % txtFilePath, 'wb') as out_csvfile:
                ...
                for i, line in enumerate(content[1:]):
                    ...
                    if s_index.isdigit():
                        ...
                        bldg_code = "%s_%s" % (sParticella,sEdificio)
                        ...
                        info_dict = {"shape_id":toFloat(s_index),"pk_min":toFloat(splitted[-4]),"pk_max":toFloat(splitted[-3]),"d_min":toFloat(splitted[-2]),"d_max":toFloat(splitted[-1].strip())}
