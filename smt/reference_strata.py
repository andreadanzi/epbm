        ...
        if pd:
            ...
            if type_of_analysis == 's' and nSamples > 0:
                ...
                for i, vloss_tail_sample in enumerate(vloss_tail_samples):
                    samples["items"].append({"vloss_tail":vloss_tail_sample, "p_tbm":std_norm_samples[i], "type":None})
                log.debug( "before assign_standard_strata_samples - len samples %d" % len (samples["items"]))
                assign_standard_strata_samples(rs_items_list,samples,nSamples)
                log.debug('gen_samples_strata, assign_standard_strata_samples terminated ')
                ...
                log.debug('gen_samples_strata, assign_custom_strata_samples terminated ')
