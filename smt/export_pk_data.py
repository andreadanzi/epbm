...
fh = logging.handlers.RotatingFileHandler('export_data.log',maxBytes=5000000, backupCount=5)
        ...
        if pd:
            ...
            for dom in found_domains:
                ...
                for aset in asets:
                    ...
                    als = db.Alignment.find({"alignment_set_id":a_set._id},{"PK":True, "VOLUME_LOSS":True, "SETTLEMENT_MAX":True, "K_PECK":True, "phi_tun":True, "ci_tun":True}).sort("PK", 1)
                    a_list = list(als)
                    header = ["PK","VOLUME_LOSS","SETTLEMENT_MAX","K_PECK","phi_tun","ci_tun"]
                    rows = []
                    for d in a_list:
                        row = [d["PK"], d.get("VOLUME_LOSS",""), d.get("SETTLEMENT_MAX",""),
                               d.get("K_PECK",""), d.get("phi_tun",""), d.get("ci_tun","")]
                        rows.append(row)
                    with open(os.path.join(sPath,sCode + "_export.csv"), 'wb') as out_csvfile:
